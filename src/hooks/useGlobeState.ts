  ...
  const handleZoomChange = useCallback(
    (newZoomLevel: number) => {
      setZoomLevel(prev => {
        ...
        if (rounded < prev - ZOOM_LEVELS.THRESHOLDS.SMOOTH_ZOOM_JUMP) return rounded;
        ...
        // 작은 변화도 반영 (더 부드러운 줌)
        if (Math.abs(prev - rounded) >= ZOOM_LEVELS.THRESHOLDS.SMOOTH_ZOOM_THRESHOLD) return rounded;
